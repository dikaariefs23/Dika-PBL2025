...
const Home = () => {
  ...
  return (
    <section className="bg-[#fffdfb] text-[#5b4636] min-h-screen overflow-hidden">
      ...
      {/* TUJUAN UTAMA */}
      <section id="tujuan-utama" className="max-w-5xl mx-auto mt-20 px-6 fade-up">
        <h2 className="font-semibold text-2xl text-[#7A5230] mb-6 text-center">Tujuan Utama</h2>
        <div className="bg-white/70 shadow-md rounded-2xl p-8">
          <ul className="list-disc list-inside space-y-2 text-left mx-auto w-fit text-base leading-relaxed">
      ...
      <section id="about" className="max-w-5xl mx-auto mt-24 px-6 fade-up text-center">
        <h2 className="text-3xl font-bold text-[#7A5230] mb-6">Tentang Kami</h2>
        <p className="text-base text-gray-700 leading-relaxed max-w-3xl mx-auto">
          Schole Fitrah hadir untuk menumbuhkan fitrah anak dan keluarga melalui pendidikan berbasis nilai, pengalaman, dan kolaborasi.
          Kami berkomitmen membangun ekosistem belajar yang alami, modern, dan sesuai dengan fitrah tumbuh anak.
        </p>
      </section>

      {/* TIGA PILAR */}
      <section id="pilar" className="max-w-6xl mx-auto text-center mt-20 px-6 fade-up">
        <h2 className="title-section mb-12 text-3xl font-bold text-[#7A5230]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 justify-items-center">
          {[
            ...
            <div
              key={i}
              className="card hover-lift transition-all duration-300 w-full md:w-80 text-center"
            >
              {pilar.icon}
              <h3 className="font-semibold text-lg text-[#7A5230] mb-2">{pilar.title}</h3>
              <p className="text-sm leading-relaxed text-gray-700">{pilar.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="programs" className="max-w-6xl mx-auto text-center mt-20 px-6 fade-up">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 justify-items-center">
          {[
            ...
            >
              {program.icon}
              <h3 className="font-semibold text-lg text-[#7A5230] mb-2">{program.title}</h3>
              <p className="text-sm leading-relaxed text-gray-700">{program.desc}</p>
            </div>
          ))}
        </div>
      </section>
